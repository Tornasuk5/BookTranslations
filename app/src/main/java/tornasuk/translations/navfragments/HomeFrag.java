...
import tornasuk.translations.StaticVariables;
import tornasuk.translations.adapters.HomeAdapter;
import tornasuk.translations.models.Novel;
import tornasuk.translations.models.Volume;
...
public class HomeFrag extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        homeBinding.txtCountTranslations.setOnLongClickListener(view -> {
            if (!StaticVariables.isTornasukAuth){
                FirebaseAuth auth = FirebaseAuth.getInstance();
                auth.signInWithEmailAndPassword("loki_mousou@hotmail.com", "tornasukpass1234")
                        .addOnSuccessListener(authResult -> {
                            Toast.makeText(requireContext(), "Permiso Tornasuk - Activado", Toast.LENGTH_SHORT).show();
                            StaticVariables.isTornasukAuth = true;
                        })
                        .addOnFailureListener(e ->
                                Toast.makeText(requireContext(), "No se ha podido iniciar sesión", Toast.LENGTH_SHORT).show());
            }
            return false;
        });
