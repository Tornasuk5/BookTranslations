...
import tornasuk.translations.models.Page;
import tornasuk.translations.models.Translation;
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for(DataSnapshot translationSnap : snapshot.getChildren()){
                    ...
                    if(!translationSnap.getKey().equals("volImg")){
                        numPg = Integer.parseInt(translationSnap.getKey().split("-")[0].replace(StaticVariables.refID, ""));
                ...
                pagAdapter.setOnLongClickPgListener(v -> {
                    ...
                    try {
                        ...
                        if (countPg == 0) {
                            String pg = StaticVariables.refID + pages.get(pagPos).getNumPage() + "-" + (1);
                            deletedPags.add(pg);
                        }
                    ...
                    if(countPg > 0){
                        for (int i = 1; i <= countPg; i++) {
                            String pg = StaticVariables.refID + pages.get(pagPos).getNumPage() + "-" + (i);
                            deletedPags.add(pg);
                        }
                    }
                    return false;
